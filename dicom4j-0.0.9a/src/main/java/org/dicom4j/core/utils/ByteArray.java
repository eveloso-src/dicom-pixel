...
public class ByteArray {
	 ...
	 * @return the unsigned integer value
	 */
	public static final int bigEndianToUnsignedInt(byte[] b, int offset,
			int length) {
		...
		// System.err.println("ByteArray.bigEndianToUnsignedInt():
		// byte[]="+HexDump.dump(b));
		int v = 0;
		while (length-- > 0) {
			v = v << 8 | b[offset++] & 0xff; // mask to prevent sign extension from
			// "signed" byte value
		}
		// System.err.println("ByteArray.bigEndianToUnsignedInt(): v="+v);
		// System.err.println("ByteArray.bigEndianToUnsignedInt():
		// v=0x"+Integer.toHexString(v));
		return v;
	}
	...
	public static final byte[] concatenate(byte[] src1, byte[] src2) {
		return src1 == null ? src2 : src2 == null ? src1 : concatenate(src1, 0,
				src1.length, src2, 0, src2.length);
	}
	 ...
	 *         array followed by the bytes extracted from the second array
	 */
	public static final byte[] concatenate(byte[] src1, int offset1, int length1,
			byte[] src2, int offset2, int length2) {
	 ...
	 */
	public static final void swapEndianness(byte[] src, int byteCount,
			int wordLength) {
		int startOfWord = 0;
		...
		while (startOfWord < byteCount) {
			// System.err.println("startOfWord="+startOfWord+" endOfWord="+endOfWord+"
			// countWithinWord="+countWithinWord);
